...
const scroll = (el) => {
    ...
    const navbarContainerHeight = document.getElementById('navbar-container').clientHeight;
    const y = el.getBoundingClientRect().top + window.scrollY - navbarContainerHeight;
    window.scrollTo({
        top: y,
        behavior: 'smooth'
    });
...
// scroll
document.getElementById('about-navbar-item').onclick = () => scroll(aboutContainer);
document.getElementById('work-history-navbar-item').onclick = () => scroll(workHistoryContainer);
document.getElementById('projects-navbar-item').onclick = () => scroll(projectsContainer);
